...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        CHK2(epoll_events_count,epoll_wait(epfd, events, EPOLL_SIZE, EPOLL_RUN_TIMEOUT));//EPOLL_RUN_TIMEOUT宏定义为-1，则阻塞
        tStart = clock();
        for(int i = 0; i < epoll_events_count ; i++)
        {
            ...
            }else //之前已经被accept过的socket，现在又有新的消息发送过来了
            {
...
int handle_message(int client)  
{
    ...
    { 
        ...
        for(it = clients_list.begin(); it != clients_list.end(); it++)//A用户发送了一条消息，则需要把这条消息，发送给除A以外的其他用户
        {
           if(*it != client)
           { 
